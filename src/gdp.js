        ...
        function draw() {
            ...
            for (let i = 0; i < 4; i++) {
                if (gdpData[i].name === city.value) {
                    ...
                    for (let j = 0; j < 17; j++) {
                        svg.append("circle").attr("cx", xPo).attr("cy", yPo.gdp[j]).attr("r", w/200).style("fill", "#66CCFF");
                        svg.append("circle").attr("cx", xPo).attr("cy", yPo.house[j]).attr("r", w/200).style("fill", "#00FFCC");
                        if (j !== 16) {
                    ...
                    for (let j = 0; j < 17; j++) {
                        ...
                        svg.append("text").attr("x", xPo - w/50).attr("y", h/2 + w/50).text(2001+j).style("font-size", w/80);
                        xPo += w/20;
                    }
                    ...
                    svg.append("text").attr("x", 6*w/8).attr("y", h/4-h/50+w/200).text("Housing prices").style("font-size", w/80);
                    svg.append("text").attr("x", 7*w/8).attr("y", h/4-h/50+w/200).text("GDP").style("font-size", w/80);
                }
            }
        }
    }
